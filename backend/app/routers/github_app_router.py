...
"""
GitHub App Router — Option 1 (no webhooks, pull-based)

What works:
  GET  /github-app/install       → redirect to GitHub App install page
  GET  /github-app/callback      → save installation after user installs
  GET  /github-app/installations → list user's linked installations
  GET  /github-app/repos         → list repos from all installations
  POST /github-app/webhook       → receives GitHub pings, does nothing (safe no-op)

What is intentionally disabled:
  - Auto-review on PR open (needs always-on server)
  - Auto-comment on PR open (needs always-on server)
  - All background webhook processing

How users get reviews:
  1. Install app (2 clicks, no PAT)
  2. Open dashboard → click Sync to fetch PRs
  3. Click a PR → click Review → AI runs
"""
...
async def start_installation(
    ...
    """
    Redirect user to GitHub to install the app.
    GitHub redirects back to /callback after install.
    We pass user ID in state so we can link the installation.
    """
    if not settings.GITHUB_APP_NAME:
...
async def installation_callback(
    ...
    We save the installation_id and link it to the user account.
...
async def list_app_repos(
    ...
    """List all repos accessible via GitHub App installations"""
    accounts = (
        db.query(GitHubAccount)
        .filter(
            GitHubAccount.user_id == current_user.id,
...
# ─────────────────────────────────────────────────────────────────────────────
# Webhook — intentional no-op for Option 1
# ─────────────────────────────────────────────────────────────────────────────
...
async def github_webhook(
    ...
    """
    Webhook endpoint — kept as a safe no-op for Option 1.

    We receive the webhook but do nothing with it.
    This prevents GitHub from logging delivery failures if a webhook URL
    is configured. Returns 200 immediately.

    To enable auto-review in the future:
      1. Deploy to always-on server (Fly.io)
      2. Replace this handler with the full implementation
    """
...
async def _sync_installation(installation_id: int, user: User, db: Session):
    """
    After install callback: get installation token, fetch GitHub user info,
    save or update GitHubAccount with the installation_id.
    """
    try:
        token = await get_installation_token(installation_id)
        client = GitHubClient(token)
