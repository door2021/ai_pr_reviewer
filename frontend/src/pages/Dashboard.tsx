...
export default function Dashboard() {
  ...
  useEffect(() => { setSuccessMsg(''); setComment(''); setShowDescModal(false); }, [selectedPR?.id]);
          ...
          <header className="h-16 bg-surface/80 backdrop-blur-xl border-b border-border flex items-center justify-between px-6 gap-4 flex-shrink-0">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <button onClick={() => navigate('/settings')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium hover:opacity-80 ${isAutoMode ? 'bg-purple-500/10 border-purple-500/20 text-purple-400'
                      ...
                      <div className="absolute right-0 top-full mt-1 w-48 bg-surface border border-border rounded-lg shadow-xl z-50 overflow-hidden">
                        {(['squash', 'merge', 'rebase'] as const).map(m => (
                          <button key={m} onClick={() => { setMergeMethod(m); setShowMergeMenu(false); }}
                            className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${mergeMethod === m ? 'bg-primary/10 text-primary' : 'text-text hover:bg-surface/80'
                              }`}>
          ...
          {!selectedPR ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-6 pt-3 pb-2 min-h-0">
                ...
                {currentReview && (
                  <div className={`mb-4 rounded-xl border overflow-hidden ${currentReview.status === 'processing'
                      ...
                      {currentReview.status === 'processing' ? (
                        ...
                        <>
                          ...
                          {currentReview.safety_score !== null && currentReview.safety_score !== undefined && (
                            <div className={`ml-auto flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${(currentReview.safety_score ?? 0) >= 80
                                ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                    ...
                    {currentReview.status !== 'processing' && currentReview.ai_feedback && (
                      <div className="px-4 py-3 space-y-3">
                        ...
                        {currentReview.ai_feedback.issues && currentReview.ai_feedback.issues.length > 0 && (
                          <div className="space-y-2">
                            ...
                            {currentReview.ai_feedback.issues.map((issue: any, i: number) => (
                              <div key={i} className={`flex gap-2.5 p-2.5 rounded-lg text-xs ${issue.severity === 'high'
                                  ? 'bg-red-500/8 border border-red-500/15'
                                  : issue.severity === 'medium'
                                    ? 'bg-yellow-500/8 border border-yellow-500/15'
                                    : 'bg-blue-500/8 border border-blue-500/15'
                                }`}>
                                <span className={`font-bold uppercase flex-shrink-0 ${issue.severity === 'high' ? 'text-red-400'
                        ...
                        {currentReview.ai_feedback.ready_for_merge !== undefined && (
                          <div className={`flex items-center gap-2 text-xs font-medium ${currentReview.ai_feedback.ready_for_merge ? 'text-emerald-400' : 'text-yellow-400'
                            }`}>
